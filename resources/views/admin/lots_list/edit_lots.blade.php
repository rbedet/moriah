...
<div class="">
    ...
    <div class="row">
        <div class="col-md-12 col-sm-12 col-xs-12">
            <div class="x_panel">
                <div class="x_title">
                    <h2>Edit Lot <a href="{{route('lots.index')}}" class="btn btn-info btn-xs"><i class="fa fa-chevron-left"></i> Back </a></h2>
                    <div class="clearfix"></div>
                </div>
                <div class="x_content">
                    <br />
                    <form method="post" action="{{ route('lots.update', ['id' => $lot->id]) }}" data-parsley-validate class="form-horizontal form-label-left">
									...
									@foreach ($lot_groups as $group)
										...
										@foreach ($lot_types as $type)
											@if ($type->lot_group_id == $group->id)
											 <option value="{{$type->id}}"  @if ($type->id == $lot->lot_type_id) selected @endif>{{$type->name}}</option>
											@endif
										@endforeach
										</optgroup>
									@endforeach
						...
						<div class="form-group{{ $errors->has('block') ? ' has-error' : '' }}">
							...
							<div class="col-md-6 col-sm-6 col-xs-12">
								<input type="text" value="{{$lot->block}}" id="block" name="block" class="form-control col-md-7 col-xs-12">
								@if ($errors->has('block'))
						...
						<div class="form-group{{ $errors->has('lot') ? ' has-error' : '' }}">
							...
							<div class="col-md-6 col-sm-6 col-xs-12">
								<input type="text" value="{{$lot->lot}}" id="lot" name="lot" class="form-control col-md-7 col-xs-12">
								@if ($errors->has('lot'))
						...
						<div class="form-group{{ $errors->has('status') ? ' has-error' : '' }}">
							...
							<div class="col-md-6 col-sm-6 col-xs-12">
								<select class="select2_group form-control" id="status" name="status">
									...
									<option value="Occupied" @if ($lot->status == 'Occupied') selected @endif>Occupied</option>
                        ...
                        <div class="form-group{{ $errors->has('description') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6 col-sm-6 col-xs-12">
                                <input type="text" value="{{$lot->description}}" id="description" name="description" class="form-control col-md-7 col-xs-12">
                        ...
                        <div class="form-group">
                            <div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-3">
                                ...
                                <button type="submit" class="btn btn-success">Save Lot Changes</button>
                            </div>
                        </div>
                    </form>
                </div>
